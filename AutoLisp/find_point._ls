(defun c:PrintEntitiesInWindow ( / p1 p2 ss i entType startPt endPt pointsList circles)
  ;; 첫 번째 점 선택
  (setq p1 (getpoint "\n첫 번째 점을 선택하세요: "))
  ;; 두 번째 점 선택
  ...
  (if ss
    (progn
      ...
      (setq circles '())    ;; 원의 정보를 저장할 리스트 초기화
      ;; 선택된 객체들을 순회하며 정보 출력
      (while (< i (sslength ss))
        (setq ent (ssname ss i))
        (setq entType (cdr (assoc 0 (entget ent))))
        
        ;; 객체의 시작과 끝점 추출 및 원 객체 확인
        (cond
          ;; LINE 객체 처리
          ((equal entType "LINE")
      ...
      (mapcar
        '(lambda (circle)
           (princ (strcat "\n중심점: "
                          (rtos (car (car circle)) 2 2) ", "
                          (rtos (cadr (car circle)) 2 2) ", "
                          (rtos (caddr (car circle)) 2 2)
                          " - 반지름: "
                          (rtos (cadr circle) 2 2))))
        circles)
